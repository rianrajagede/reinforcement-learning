...
class BlackJack(object):
    ...
    this class only can be accessed from act(), reset(), stat()
    ...
    (stat) is a tuple of player score, dealers score, usable ace condition, 
                        done or not, player deck, dealer deck
    ...
    def stat(self):
        return self.sum_hand(self.player), self.sum_hand(self.dealer), \
                self.usable(self.player), self.done, self.player, self.dealer
...
def print_stat( pl_score, de_score, use_ace, done, player, dealer, reward=0):
    if done:
        print "== Game Over =="
        print "Reward: {}".format(reward)
    print "Player: {} | Dealer: {} | Usable Ace: {}".format(
                pl_score, de_score, use_ace)
    print "Player Deck: {}".format(player)
    print "Dealer Deck: {}".format(dealer)

def act(hit, env=env):
    stat, reward = env.act(hit)
    pl_score, de_score, use_ace, done, player, dealer = stat
    print_stat(pl_score, de_score, use_ace, done, player, dealer, reward)
